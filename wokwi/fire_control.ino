...
volatile uint8_t home_isr_guard = 0;   // 防重入计数
...
// --- 弹匣 ---
MagState     mag_state = MAG_PRESENT;  // Wokwi: 默认有弹匣
uint16_t     mag_capacity = 30;
...
void nfc_init() { /* Wokwi: no NFC hardware */ }
...
void setup() {
  ...
  // OLED 初始化 (Wokwi 可能是 0x3C 或 0x3D, 双地址尝试)
  ...
  Serial.println(F("FCU Wokwi Ready"));
  ...
  case FIRING: {
    ...
    if (home_isr_flag) {
      ...
      if (fire_mode == AUTO && trigger_now && ammo > 0 && !auto_brake_pending) {
        keep_running = true;
      } else if (fire_mode == BURST && burst_remaining > 1 && ammo > 0) {
        burst_remaining--;
        keep_running = true;
      }
