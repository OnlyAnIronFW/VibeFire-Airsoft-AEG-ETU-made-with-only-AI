...
// ============================================================
// 开机自动检测电池串联电芯数 (2S-6S)
// 基于标称电压 3.7V/cell，阈值设在 cell 区间中间
// ============================================================
...
void run_menu() {
  ...
  case MENU_CELL_CUTOFF: {
    int delta = 0;
    if (up || up_rep)       delta = +1;   // +0.05V per click
    if (down || down_rep)   delta = -1;
    if (delta != 0) {
...
void draw_main() {
  ...
  // ── 顶栏: 电压 | ROF/预压 | 状态 ──
  oled.setCursor(0, 0);
  if (bat_low) oled.print("SD!");      // shutdown: low-voltage lock
